...
const editSaveHandler = event => {
  //ств можливість редагувати івент
  ...
  if (newEvent.id === "0") {
    ...
    addNewEvent(rest) // .then(() => {
    // })
    .catch(error => console.log('save error'));
    renderE(); //тут вже працює рендер зміненого івенту але лише після обновлення сторінки    
  } else {
    const {
      id,
      ...rest
